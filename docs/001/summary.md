...
## 문제 발생
- Instructor 계정으로 로그인 후에도 인증 상태가 서버 미들웨어에 전달되지 않아 무한 리다이렉션 발생
- API 엔드포인트를 통한 로그인 방식에서는 브라우저 쿠키가 올바르게 설정되지 않아 서버 미들웨어가 인증 상태를 인식하지 못함

## 문제 분석
- 기존 로그인 방식은 API 호출 후 클라이언트가 토큰을 로컬스토리지에 저장하고 있었으나, 서버 미들웨어는 쿠키 기반 인증 상태만 인식
- Supabase SSR 패키지는 브라우저에서 직접 인증 함수를 호출할 때 쿠키를 자동으로 관리

## 해결 방안
- 로그인 페이지에서 직접 Supabase 클라이언트를 사용하여 인증 처리
- 서버 미들웨어가 인증 상태를 올바르게 인식할 수 있도록 브라우저 쿠키를 활용

## 수정 내용
- `src/app/login/page.tsx` 파일을 클라이언트 컴포넌트로 유지하고, 직접 Supabase 클라이언트를 사용하여 로그인 처리
- API 호출 대신 브라우저에서 직접 `supabase.auth.signInWithPassword()` 함수 호출
- 인증 성공 시 Supabase가 자동으로 쿠키를 설정하여 서버 미들웨어가 인증 상태를 인식
- 로그인 성공 후 `router.push("/dashboard")`로 이동하여 적절한 리다이렉션 처리

이제 Instructor 계정으로 로그인하면 서버 미들웨어가 인증 상태를 올바르게 인식하여 `/instructor/dashboard`로 리다이렉션됩니다.
